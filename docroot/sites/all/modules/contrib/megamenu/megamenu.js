  ...
  if(activeParent != -1)
  {
	...
	function megamenu_sizer(){
		...
		for ( var k=0 ; k < megaBins.length ; k++ ) {
			...
			if(megaBins.eq(k).hasClass('megamenu-slots-columnar')) {
				...
				*/
				...
				/* CUSTOM CODE MODIFICATION */
                                megaBins.eq(k).css( 'width' , $('.zone-branding').width() );
                                megaBins.eq(k).width($('.zone-branding').width());
			}
			else {
			...
			/* CUSTOM CODE MODIFICATION */
			if($(window).width() > $('.zone-branding').width()){
				var edgeOverlap = -1 * ( megaBins.eq(k).offset().left - (($(window).width() - $('.zone-branding').width())/2) );
			}
			else{
				var edgeOverlap = -1 * megaBins.eq(k).offset().left;
			}
